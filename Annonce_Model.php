...
class Annonce_Model extends My_Model {
    ...
    function getByIdSousregion($id_sousregion) {
        $query = $this->db->query("SELECT * FROM `annonce` LEFT JOIN annonce_trad on annonce_trad.id_ann=annonce.id_ann LEFT JOIN langue on annonce_trad.id_langue=langue.id_langue where annonce.id_sousregion='" . $id_sousregion . "'");
        $langue = 'French';
        $rset = $query->result();
        $rep = array();
        if (isset($rset) && !empty($rset)) {
