...
class Statut_Type_Model extends MY_Model {
    ...
    function getAll($langue) {
        $query = $this->db->query("SELECT id_statut_type, statut_trad.id_statut, type_trad.id_type, statut_trad.val AS val_statut, type_trad.val AS val_type FROM `statut_type` LEFT JOIN `statut_trad` ON `statut_type`.`id_statut` = `statut_trad`.`id_statut` LEFT JOIN `type_trad` ON `statut_type`.`id_type` = `type_trad`.`id_type` LEFT JOIN langue ON statut_trad.id_langue = langue.id_langue AND type_trad.id_langue = langue.id_langue WHERE langue = '".$langue."'");
        $rset = $query->result();
        $rep = array();
    ...
    function modify($statuttype) {
        ...
        $object_trad = array('id_statut' => $statuttype->getId_statut(), 'id_type' => $statuttype->getId_type());
