...
class Citation_Model extends My_Model {
    ...
    function getAll($langue) {
        /*$query = $this->db->query("SELECT * FROM `Citation` c LEFT JOIN langue on =langue.id_langue where langue='" . $langue . "'");
        $rset = $query->result();
        $rep = array();
        ...
        return $rep;*/
        $rep= '';
        if($langue == 'french') $rep = 'Votre confiance et notre expérience pour des transactions réussies!';
        else $rep='Your trust and our experience for successful transactions!';
