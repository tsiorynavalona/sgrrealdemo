...
class Mesure_Model extends My_Model {
    ...
    function getById($id, $langue, $annonc_id = '') {
		if($annonc_id == '' || $annonc_id == '0'){
			$annonc_id = 1;
		}
        $query = $this->db->query("SELECT * FROM `mesure` LEFT JOIN `mesure_trad` ON `mesure`.`id_mesure` = `mesure_trad`.`id_mesure` LEFT JOIN langue on mesure_trad.id_langue=langue.id_langue where langue='" . $langue . "' and mesure.id_mesure=" . $annonc_id);
        $rset = $query->result();
        if (isset($rset) && !empty($rset)) {
            $rset = $rset[0];
            $rep = new Mesure();
