    ...
    function getAll($langue) {
        ...
        if (isset($rset) && !empty($rset)) {
            foreach ($rset as $rec) {
               ...
               // $temp->setPrix(toMoney($rec->prix));
			   $temp->setMeasure_Terrin($rec->measure_terrin);
			$temp->setMeasure_Habit($rec->measure_habit);
				$temp->setPrix($this->toMoney($rec->prix));
                $temp->setAdresse_propriete($rec->adresse_propriete);
                $temp->setImages($this->getImages($temp));
                $temp->setIcones($this->getIcones($temp, $langue));
        ...
        if (!empty($page)) {
            ...
            $query = $this->db->query("SELECT
                                        * FROM `annonce`
                                        ...
                                        where langue='" . $langue . "' and etat=1 order by annonce_trad.id_ann desc limit " . $skip . "," . $nb);
        } else {
            $query = $this->db->query("SELECT
                                        * FROM `annonce`
                                        LEFT JOIN annonce_trad
                                        ...
                                        LEFT JOIN langue
                                        on annonce_trad.id_langue=langue.id_langue
                                        where langue='" . $langue . "' and etat=1 order by annonce_trad.id_ann");
        }
        $rset = $query->result();
        $rep = array();
        if (isset($rset) && !empty($rset)) {
...
//        exit;
    ...
    function searchMenu($idtype, $idstatut, $langue, $nb = '', $page = '') {
        ...
        $this->db->order_by('annonce.id_ann','desc');
...
//        exit;
        ...
        $this->db->like('langue', $langue);
        $this->db->like('etat', true);
        if (isset($crit["id_type"]) && !empty($crit["id_type"]))
            $this->db->where('id_type', $crit["id_type"]);
        if (isset($crit["id_statut"]) && !empty($crit["id_statut"]))
            $this->db->where('id_statut', $crit["id_statut"]);
            $this->db->like('reference', $crit["reference"]);
        if (isset($crit["id_sousregion"]) && !empty($crit["id_sousregion"]))
            $this->db->where('annonce.id_sousregion', $crit["id_sousregion"]);
        if(isset($crit['id_region']) && !empty($crit['id_region']))
            $this->db->where('sous_region.id_region', $crit["id_region"]);
        $sql = $this->db->get_compiled_select();
    ...
    function searchByKeywords($keyword, $langue, $nb = '', $page = '') {
        ...
        if (!empty($keywords)) {
            ...
            $this->db->or_like('reference', $keywords[0]);
        ...
        $sql = $this->db->get_compiled_select();
$sql = str_replace("WHERE", "WHERE etat = 1 AND (", $sql);
$sql = str_replace("GROUP BY", ") GROUP BY", $sql);
    ...
    function searchByCrit($criteres, $langue, $nb = '', $page = '') {
        ...
        $this->db->join("annonce_trad", "annonce.id_ann=annonce_trad.id_ann");
        $this->db->join("langue", "langue.id_langue=annonce_trad.id_langue");
        $like = array("id_type" => $criteres["id_type"], "etat" => 1, "id_statut" => $criteres["id_statut"], "annonce.reference" => $criteres["reference"], "langue" => $langue);
        $this->db->like($like);
        $this->db->like('etat', 0);
        $this->db->where("sous_region.id_sousregion",$criteres["id_sousregion"]);
        $sql = $this->db->get_compiled_select();
        ...
        if (isset($rset) && !empty($rset)) {
            foreach ($rset as $rec) {
                ...
                $icones = $this->getIcones($temp, $langue);
...
//        exit;
    ...
    function getIcones($annonce, $lang="french") {
        ...
        if (isset($rset) && !empty($rset)) {
            foreach ($rset as $rec) {
				...
				if($lang == "english"){
					$temp->setNom_icone($rec->nom_icone_eng);
				}else{
                ...
                if ($rec->id_mesure != '') {
                    if($rec->id_icone == 3){
						$temp->setMesure($this->Mesure_Model->getById($rec->id_mesure, 'french', $annonce->getMeasure_Terrin()));
					}else{
						$temp->setMesure($this->Mesure_Model->getById($rec->id_mesure, 'french', $annonce->getMeasure_Habit()));
					}
                }
                $rep[] = $temp;
            }
    ...
    function getCaracteristiques($annonce, $langue) {
        ...
        if (isset($rset) && !empty($rset)) {
            foreach ($rset as $rec) {
                ...
                $temp->setVal($rec->val);
				if($langue == "english"){
					$temp->setVal_car($rec->val_car_eng);
				}else{
   ...
   return  $sign .($j ? substr($i,0, $j) + $t : '').preg_replace('/(\d{3})(?=\d)/',"$1" + $t,substr($i,$j)) ;
    ...
    function saveIcones($annonce) {
        ...
        foreach ($icones as $icone) {
			...
			$query = $this->db->query("SELECT * FROM `icone_annonce` where val_icone='" . $icone->getVal_icone() . "' and id_ann=".$annonce->getId_ann() . " AND id_icone = " . $icone->getId_icone());
			$curan = $query->result();
			if($curan == null){
